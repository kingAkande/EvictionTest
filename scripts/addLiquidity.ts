...
async function main() {
    ...
    const  amountADesired = ethers.parseUnits("1000", 18);
    const  amountBDesired = ethers.parseUnits("500", 18);

    const  amountAMin = ethers.parseUnits("300", 18);
    const  amountBMin = ethers.parseUnits("200", 18);
    ...
    await SHFL_Contract.approve(ROUTER,  amountBDesired);
